...
class DummyHarmonicRule(Rule):
    ...
    def apply(self,context):
        # Get 'harmonic' notes (either simultaneous or overlapping notes
        # depending on the re-definition of get_harmonic_notes)
        harmonic_notes = self.get_harmonic_notes(context)

        harmonic_intervals = context.work_browser.get_harmonic_intervals(
                                context.note,harmonic_notes)

        # Remove unisons (1)
        harmonic_intervals = [inter for inter in harmonic_intervals if
                                inter != 1]

        # Remove intervals that are already present in the figured_bass
        harmonic_intervals = [inter for inter in harmonic_intervals if not
                        context.figured_bass.has_interval(context.note,inter)]

        # Dummy rule: add any harmonic intervals
        if harmonic_intervals:
            self.applicability = (self.applicability_multiplier *
                                    MAX_APPLICABILITY)
            self.addition = MultipleIntervalAddition(
                                context.note,harmonic_intervals)
        else:
...
class DummyMelodicRule(Rule):
    ...
    def interval_to_add(self,context):
        ...
        harmonic_notes = context.work_browser.get_overlapping_notes(
                                context.note)
        harmonic_intervals = context.work_browser.get_harmonic_intervals(
                                context.note,harmonic_notes)
        harmonic_intervals = [inter for inter in harmonic_intervals if
    ...
    def apply(self,context):
        # Dummy rule: if the melodic interval with previous note is <=4,
        # add some harmonic interval
        previous_notes = context.work_browser.get_previous_notes(context.note,1)

        try:
            # [0] will raise an exception for the first note as it does not
            # have a previous note
